...
- **Stack** : Astro (rendu statique) + intégration `@astrojs/vue` pour les îlots interactifs, TypeScript, npm.
- **Hébergement** : Netlify (build `npm run build`, dossier publié `dist/`).
...
Arborescence cible (Astro standard) :

```
src/
  pages/        routes (fichiers .astro, kebab-case)
  layouts/      gabarits de page (.astro)
  components/   composants .astro (statiques) et .vue (interactifs)
  content/      collections de contenu (projets, services, équipe…)
...
> Le projet n'est pas encore initialisé. Après le scaffolding, mettre à jour cette section et la section 2 avec la réalité.

## 2. Commandes

Prévues (scripts Astro standard, à confirmer après le scaffolding) :
...
## 8. Conventions de code (proposées, projet vierge)
...
- **Contenu** : les données (projets, services, équipe, témoignages) vivent dans des collections `src/content/` typées, pas en dur dans les composants.
- **Styles** : `<style>` scopé par composant ; tokens (couleurs, typo, espacements) en variables CSS dans `src/styles/`. Pas de valeurs magiques répétées.
